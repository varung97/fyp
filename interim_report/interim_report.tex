...
\begin{document}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            ...
            \State $roots := rootsOfSubtrees(c_1)$

            \State $lowerBoundary := \bigcup_{c \in children(u) - \{c_1\}} rootsOfSubtrees(\leafset(T_A[c]))$

            \IIf{$\leafset(T_B[l_{c_1}]) \in roots$}
                {$candidates := candidates \cup path(parent^{T_B}(l_{c_1}), l_u)$}
                $candidates := candidates \cup path(l_{c_1}, l_u)$
    ...
    \begin{incompatibilityrootsofsubtrees}
        ...
        % TODO: Can probably define l_u here
