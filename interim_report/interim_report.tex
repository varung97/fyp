...
\begin{document}
    ...
    \begin{algorithm}[!ht]
        ...
        \begin{algorithmic}[1]
            ...
            \ForAll{$\tau \in \sigma(\TA)$}
                ...
                \State $sideCover^{\TB}(p_i) := sideCover^{\TB}(p_i) \cup sideCover'$, where $\tau$ is associated with $p_i \in \pi(\TA)$
            \EndFor
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            \Input Given some tree $T$, cluster $C \subseteq L$ and some node $u \in V(T)$ where $\leafset^{T}(u) \cap C = \emptyset$. Let $Cu = C \cup \leafset^{T}(u)$. The algorithm takes in the set $I = incompatible^{T}(C)$ and the nodes $u$, $v = coverer^{T, Cu}(u)$, $l_C = lca^{T}(C)$ and $l_{Cu} = lca^{T}(Cu)$.
