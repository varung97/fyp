...
\begin{document}
    ...
    \begin{incompatibilityrecursive}
        ...
        \begin{proof}
            ...
            \textit{Case 3.} $v \in incompatible^{\TB}(c_1)\ -\ \bigcup_{r \in removedRST^{\TB}(u, c_1)} pathExc^{\TB}(r, l_{c_1}^{\TB})$ and\\[0.25em] % Overflows otherwise
    ...
    We thus present Algorithm~\ref{alg:computerootsofsubtrees} (on the following page) which, for some internal node $u \in V(\TA)$ and $c_1 \in children(u)$, takes in $RST^{\TB}(c_1)$, $LB^{\TB}(u, c_1) = \bigcup_{c \in children(u) - \{c_1\}} RST^{\TB}(c)$ and $\TB$ where $pci^{\TB}(\leafset^{\TA}(c_1)))$ holds and returns $newRST^{\TB}(u, c_1)$, $removedRST^{\TB}(u, c_1)$ and $oldRST^{\TB}(u, c_1)$. In addition $\TB$ is updated so that $pci^{\TB}(\leafset^{\TA}(u))$ holds.

    The algorithm iterates upwards from each node in $LB$, updating $counter$ values, and in doing so discovering which nodes are compatible with $\leafset^{\TA}(u)$. We prove the correctness of this algorithm below.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            \Input For some trees $\TA$ and $\TB$, for some internal node $u \in V(\TA)$ and $c_1 \in children(u)$, $RST^{\TB}(c_1)$, $LB^{\TB}(u, c_1)$ and $\TB$ where $pci^{\TB}(\leafset^{\TA}(c_1))$ holds with $\leafset^{\TA} = \leafset{\TB} = L$.

            \Output $newRST^{\TB}(u, c_1)$, $removedRST^{\TB}(u, c_1)$ and $oldRST^{\TB}(u, c_1)$.

            \SideEffect $pci^{\TB}(\leafset^{\TA}(u))$ holds.

            \State $new, removed := \{\}$

            \ForAll{$v \in LB^{\TB}(u, c_1)$}
                \State $counter^{\TB}(parent(v)) := counter^{\TB}(parent(v)) + 1$

                \State $v := parent(v)$

                \While{$counter^{\TB}(v) = |children(v)|$}
                    \State $counter^{\TB}(parent(v)) := counter^{\TB}(parent(v)) + 1$

                    \State $new := (new \cup \{v\}) - children(v)$

                    \State $removed := removed \cup (children(v) \cap RST^{\TB}(c_1))$

                    \State $v := parent(v)$
                \EndWhile
            \EndFor

            \State $old := RST^{\TB}(c_1) - removed$
    ...
    \begin{computerootsofsubtreescorrectness}
        ...
        For any trees $\TA$ and $\TB$, for any internal node $u \in V(\TA)$ and $c_1 \in children(u)$, the algorithm\\ % Overflows otherwise
        \texttt{Compute\_Roots\_Of\_Subtrees}$(RST^{\TB}(c_1), LB^{\TB}(u, c_1), \TB)$ outputs $newRST^{\TB}(u, c_1)$, $removedRST^{\TB}(u, c_1)$ and\\ % Overflows otherwise
        $oldRST^{\TB}(u, c_1)$.
        ...
        \begin{proof}
            ...
            Thus our invariants hold. Since $D_{\delta} = \leafset^{\TA}(u)$, the output of\\ % Overflows otherwise
    ...
    \begin{computerootsofsubtreesruntime}
        ...
        For any trees $\TA$ and $\TB$, for any internal node $u \in V(\TA)$ and $c_1 \in children(u)$, the algorithm\\ % Overflows otherwise
        \texttt{Compute\_Roots\_Of\_Subtrees}$(RST^{\TB}(c_1), LB^{\TB}(u, c_1), \TB)$ runs in\\ % Overflows otherwise
