...
% 2. Do we really need side effects??
% 3. Diagrams would be helpful!!
% 4. Need a better introduction - what's the motivation behind this problem?
% 5. Lemmas showing correctness of algorithm - some sort of recursive formulation
% 6. Add r_{\alpha} to rootsOfSubtrees also?
% 7. No inIDS queries any longer
% 8. Will need to process all side trees first
        ...
        \begin{algorithmic}[1]
            ...
            \For{$i := 2$ \textbf{to} $\alpha$}
                ...
                \ForAll{side trees $\tau$ associated with $p_i$}
                    \ForAll{$x \in$ \texttt{Filter\_Clusters\_Helper}($\tau$, $T_B$)}
                ...
                \ForAll{$x \in lowerBoundary$}
                    ...
                    \State Insert $x, maxWeight(\pi(x, r_i))$ into $IDS$
                    ...
                    \While{$counter(x) = |\leafset(T_B[x])|$ and $x \neq r$}
                        ...
                        \State Insert $x, maxWeight(path(x, r_i))$ into $IDS$
                    \EndWhile
                \EndFor
            ...
            \State \algorithmicforall\ $x \in rootsOfSubtrees$, $counter(parent^{T_B(x)}) := 0$

            \State \Return $rootsOfSubtrees$
        \end{algorithmic}
    \end{algorithm}
