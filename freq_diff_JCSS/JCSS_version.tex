...
%        \end{proof}
    ...
    \begin{algorithm}[!ht]
		...
		\State Remove nodes $u \in addNodes$ and recursively remove the descendants of $u$ in $Conflict$
	...
	{\bf Varun: This algorithm might become harder to analyse because we need to construct all the restricted subtrees together - if we work one node at a time, then this analysis becomes harder since we're only constructing the restricted subtrees for a subset of all the side trees of $\TA$.}
