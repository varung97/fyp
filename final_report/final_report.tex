...
\begin{document}
    ...
    This report concludes a four year long journey that has been in turns incredibly rewarding and immensely frustrating. And as I approach the end of my undergraduate degree, it would be remiss of me to fail to thank the following individuals, without whom it would have been impossible for me to get through this.
    ...
    Second, my friends - Srishti, whose maturity has ever been my anchor; Advay, whom I should have long since appointed my official decision maker; Chandraanshu, who soaked up innumerable rants with nary a complaint; Avnika and Ishanee, who continue to tolerate me for reasons unknown; Shailesh, Askhat and Tanvi, who play the best game of cards I have seen; Pankaj, whose boundless enthusiasm is an instant pick-me-up; and all the rest, with whom countless memories were made.
    ...
    A variety of techniques of constructing a consensus tree from a set of phylogenetic trees have been developed over the last half century, starting with the Adams consensus tree \citep{adams1972consensus} in 1972. There is a strong motivation for the development of different types of consensus trees since they each have their own benefits and drawbacks. Some of these techniques are summarised in \S 6.2 of \citep{bryant1997hunting} and a comparative analysis can be found in \citep{bryant2003classification}. We illustrate some techniques here. The \textit{strict consensus tree} \citep{sokal1981taxonomic} only keeps those clades that occur in all the input trees. While easily reconstructed and interpreted, it can discard a lot of potentially important information if there is a lot of disagreement between the various trees. The \textit{majority rule consensus tree} \citep{margush1981consensusn} is a generalisation of the previous, allowing all clades that occur in a majority of the trees. \cite{holder2008justification} showed that these trees are optimal given a certain context.

    This article studies a specific type of consensus tree, known as the \textit{frequency difference consensus tree} \citep{goloboff2003improvements}. The \textit{frequency difference consensus tree} (abbreviated henceforth as FDCT) further generalises the \textit{majority rule consensus tree}, keeping every clade that occurs in more trees than the most frequent clade that contradicts it (the concept of contradiction is formalized in Subsection~\ref{subsec:def}). The FDCT is motivated by a desire to design an effective criterion for determining which clades are strongly supported within a set of trees. As set out in \citep{goloboff2003improvements}, although the \textit{majority rule consensus tree} includes a clade that is supported by 60\% of a set of trees and contradicted by 40\% of them, it does not include a clade supported by 40\% of the trees but not contradicted by any of them. A possible resolution to this is to define strongly supported clades as those with greater frequency than all clades incompatible with them. These are called frequency difference clades and all of these are included in the FDCT.

    \cite{dong2010majority} provided a comparison of the FDCT and a few other types of consensus trees. Barrett et al. \cite{barrett2013plastid} employed the idea of using frequency difference as a measure of clade support while analysing angiosperm phylogeny and commented ``[the] frequency difference metric ... is particularly useful for assessing support when it is not overwhelmingly in favour of one particular topology, and should be especially useful in phylogenomic analyses in general, where hundreds or thousands of characters may contribute to branch support''. \cite{steel2014axiomatic} investigated a generalisation of the FDCT to \textit{supertrees}, i.e. consensus trees built from input trees that do not necessarily have the same leaf sets. They show that, unlike some other popular techniques, the FDCT definition easily generalises to a viable supertree definition and conclude that the FDCT is ``worthy of more widespread usage and serious study''. Given that the FDCT and the frequency difference measure have been utilised in various studies over the years \citep{garcia2014testing,barrett2013plastid,molineri2010cladistic,molineri2013phylogeny,molineri2015phylogeny,lindqvist2006molecular,han2014new} and keeping in mind the favourable opinions above, we set out to improve upon the best known deterministic algorithm for reconstructing the FDCT of a set of trees.
    ...
    We restate the following lemma outlining the \textit{lca} operation from \citep{bender2000lca}:
    \newline
    ...
    For any array $A[1 ... n]$ of length $n$ and any indices $i, j$, $1 \leq i \leq j \leq n$, let $rmq^A(i, j)$ denote $max_{i \leq k \leq j}A[k]$. We restate the following lemma about answering $rmq$ queries from \citep{bender2000lca}:
    \newline
    ...
    For any tree $T$ and cluster $C \subseteq \leafset(T)$, define the set $incompatible^{T}(C) = \{u : u \in V(T) \text{ and } \leafset^{T}(u) \not\compatible C\}$. Lemma 6 of \citep{jansson2018algorithms}, which gives a property of this set, is restated here since it is crucial in the development of an algorithm discussed below.
    \newline
    ...
    We restate the following lemma outlining the \texttt{Merge\_Trees} operation from \citep{jansson2016improved}:
    \newline
    ...
    Theorem 3 of \citep{jansson2018algorithms} gives the following corollary:
    \newline
    ...
    Given the set of trees $\mathcal{S} = \{T_1, T_2, ..., T_k\}$, where $\leafset(T_1) = \leafset(T_2) = ... = \leafset(T_k) = L$ and $n = |L|$, the weighting step computes $\weight(u)$ for every node $u \in \bigcup_{T \in \mathcal{S}}V(T)$. Recall that $\weight(u)$ gives the frequency of $\leafset(u)$ in $\mathcal{S}$. We divide this work (as in \citep{gawrychowski2017faster}) into the labelling and counting steps. The labelling step assigns a label to each node $u \in T, T \in \mathcal{S}$, denoted by $id(u)$, such that $id(u) \in [1 ... 2kn]$ and for any node $u' \in T', T' \in \mathcal{S}$, $id(u) = id(u')$ iff $\leafset^T(u) = \leafset^{T'}(u')$. That is, two nodes have the same label iff they are associated with the same cluster. The counting step sorts these labels, allowing us to count how many nodes exist with each label, giving us the frequencies (or the weights) of the nodes.

    Recall that the approach in \citep{gawrychowski2017faster} costs $O(kn\,log^2n)$ time. We develop a way to achieve $O(kn\,log\,n)$ time.
    ...
    Given a tree $T$ leaf-labelled by $L$ and a cluster $C \subseteq L$, define a set $M \subseteq V(T)$ to be a cover of $C$ in $T$ if $\bigcup_{u \in M} \leafset^{T}(u) = C$. Then we define the minimum cover of $C$ in $T$, denoted as $minCover^{T}(C)$, to be the smallest set $M$ such that $M$ is a cover of $C$. Note that this set is well defined for each cluster. Further, for any node $u \in V(T)$ such that $\leafset^{T}(u) \subseteq C$, define the node covering $u$, denoted as $coverer^{T, C}(u)$, to be the ancestor of $u$ which belongs in $minCover^{T}(C)$. Observe that this node must exist, since $\leafset^{T}(u)$ must be covered, and covering it with descendants of $u$ certainly yields a larger cover than if we use $u$. Figure~\ref{fig:mincoverrecursive}(a) gives an example of a minimum cover. In the same diagram, let $C = \{a, b, c, f, g, h\}$ and $v$ be as labelled. Then $coverer^{T, C}(v) = w$ and $minCover^{T}(C)$ is given by all the circled nodes.

    \begin{figure}[ht]
        \includegraphics[scale=0.48]{mincoverrecursive}
        \centering
        \caption[Minimum cover and the $addToCover$ operation]{Illustration of minimum cover and $addToCover$ operation. Let $C = \{a, b, c, f, g, h\}$. Then the circled nodes in (a) give $minCover^{T}(C)$. Take the leaf $e$, marked with a square in (a) and let $Ce = C \cup \{e\}$. Then $coverer^{T, Ce}(e) = u$, marked with a square in (b). Then the circled node and $u$ in (b) give $minCover^{T}(Ce)$.}
        \label{fig:mincoverrecursive}
    \end{figure}

    Given some tree $T$ leaf-labelled by $L$, cluster $C \subseteq L$ and node $u \in V(T)$ such that $\leafset^{T}(u) \cap C = \emptyset$, let $M = minCover^{T}(C)$ and $Cu = C \cup \leafset^{T}(u)$. Then it is useful to be able to find $coverer^{T, Cu}(u)$. To this end, we define the operation $findCoverer^{T}(M, u)$ which returns $coverer^{T, Cu}(u)$. It is also of interest to define an operation that can then find the minimum cover of $Cu$ using this. Thus, we define the operation $addToCover^{T}(M, u, v)$, where $v = coverer^{T, Cu}(u)$, which returns $minCover^{T}(Cu)$.

    Figure~\ref{fig:mincoverrecursive} demonstrates how these queries work. Here, $C = \{a, b, c, f, g, h\}$ and we take the leaf $e$. The circled nodes in Figure~\ref{fig:mincoverrecursive}(a) give $minCover^{T}(C)$. Then we wish to compute $coverer^{T, Ce}(e)$, where $Ce = C \cup \{e\}$. The idea for doing so will be to iterate upwards from $e$ while the leafset of the node we are at is a subset of $Ce$. Thus, since $\leafset^{T}(parent(e)) \subseteq Ce$, we go upwards one level. We can then go a further  level, to the node $u$ since $\leafset^{T}(u) \subseteq Ce$. At this point we cannot go any further up. Hence, we say that we have found $u = coverer^{T, Ce}(e)$. Now to find $minCover^{T}(Ce)$, notice that we can subtract the set of all children of the nodes on the path from $e$ to $u$ (excluding $e$) from $M$, add $u$, and this gives us the required set. That is, we take $M \cup \{u\} - \bigcup_{u' \in path^{T}(e, u]} children(u')$. From these observations, we give the following recursive formulation of the two queries.
    \newline
    ...
    \begin{mincoverrecursive}
        ...
        Given a tree $T$ leaf-labelled by $L$, a cluster $C \subseteq L$ and a node $u \in V(T)$ such that $\leafset^{T}(u) \cap C = \emptyset$ let $M = minCover^{T}(C)$, $Cu = C \cup \leafset^{T}(u)$ and $v = coverer^{T, Cu}(u)$. Then
         \[addToCover^{T}(M, u, v) = M \cup \{v\} - \bigcup_{w \in path^{T}(u, v]} children(w)\]
    ...
    Subsection~\ref{subsec:addtocover} details how to actually implement these. We now make a claim (proved later) about the running time of these operations.
    \newline

    \begin{mincoverruntime}
        \label{lem:mincoverruntime}

        Given a tree $T$ leaf-labelled by $L$, a cluster $C \subseteq L$ and a node $u \in V(T)$ such that $\leafset^{T}(u) \cap C = \emptyset$, let $M = minCover^{T}(C)$, $Cu = C \cup \leafset^{T}(u)$ and $v = coverer^{T, Cu}(u)$. Then the operations $findCoverer(M, u)$ and $addToCover^{T}(M, u, v)$ can be performed in amortized $O(1)$ time.
    ...
    We now wish to define an operation similar to $addToCover$, which takes the set of nodes incompatible with a given cluster and updates this set to become those incompatible with a superset of that cluster. Formally, given tree a $T$ leaf-labelled by $L$, a cluster $C \subseteq L$ and node $u \in V(T)$ such that $\leafset^{T}(u) \cap C = \emptyset$, we would like to have an operation that takes in $I$ and $u$ (along with some other arguments) and returns $incompatible^{T}(C \cup \leafset^{T}(u))$. Figure~\ref{fig:incompatibility}(b) shows the result of an $updateIncompatible$ query.
    ...
    Thus we define the operation $updateIncompatible^{T}(I, u, v, l_C, l_{Cu})$ which returns $incompatible^{T}(Cu)$. The actual implementation of this operation is detailed in Subsection~\ref{subsec:updateincompatible}.
    ...
    The $|incompatible^{T}(Cu) - incompatible^{T}(C)|$ term in the runtime comes the nodes we add to $I$, while the $|incompatible^{T}(C) - incompatible^{T}(Cu)|$ term is due to deleting nodes from $I$. The $log\,n$ factor comes about due to the data structure used to store $I$; this is explained in detail later.
    ...
    Recall that \texttt{Filter\_Clusters} takes in trees $\TA$ and $\TB$ and returns a tree $T$, where $\mathcal{C}(T) = \{C : C \in \mathcal{C}(\TA) \text{ and } \weight(C) > max(\{\weight(C_1) : C_1 \in \mathcal{C}(\TB) \text{ and } C_1 \not\compatible C\})\}$. However, during recursive calls to this function, it could be that we delete the root of $\TA$. Thus, we change the signature slightly to return a forest $\mathcal{F}$ such that the clusters present in this forest are exactly the required ones, i.e. $\bigcup_{T' \in \mathcal{F}} \mathcal{C}(T') = \{C : C \in \mathcal{C}(\TA) \text{ and } \weight(C) > max(\{\weight(C_1) : C_1 \in \mathcal{C}(\TB) \text{ and } C_1 \not\compatible C\})\}$. Note that since the root of the original $\TA$ is a trivial cluster, it is compatible with all other clusters, so it will never be deleted. Thus, the outermost recursive call will yield only a single tree. We also modify the signature to return the set $minCover^{\TB}(\leafset^{\TA})$.

    Algorithm~\ref{alg:filterclusters} gives the algorithm \texttt{Filter\_Clusters}. Step~\ref{step:centroidpath} decomposes $\TA$ into a \textit{centroid path} $\langle p_{\gamma}, p_{\gamma - 1}, \dots, p_1 \rangle$, where $p_{\gamma}$ is the root of $\TA$, and a set of \textit{side trees} (similarly to \citep{jansson2018algorithms}). Observe that for any side tree $\tau \in \sigma(\TA)$, $|\leafset^\tau| \leq n/2$ and that $\{\leafset^{\tau} : \tau \in \sigma(\TA)\}$ forms a partition of $L\, \backslash\, {p_1}$. We further note that for any internal node $p_i \in \pi(\TA)$, $\leafset^{\TA}(p_{i - 1}) \subset \leafset^{\TA}(p_i)$, i.e. the cluster associated with $p_{i-1}$ is a proper subset of $p_i$. This then leads to the intuition behind the procedure \texttt{Filter\_Clusters} - recursively run the algorithm on the side trees, and then iterate over the centroid path from leaf to root, finding $incompatible^{\TB}(p_i)$ for each $p_i \in \pi(\TA)$.

    More specifically, Step~\ref{step:obtainsidecover} first obtains the nodes that cover, for each $p_i \in \pi(\TA)$, each of the side trees of $p_i$ using recursive calls to \texttt{Filter\_Clusters}, i.e. the set $\bigcup_{\tau \in \sigma(p_i)} minCover^{\TB}(\leafset^{\tau})$. This set is denoted by $sideCover^{\TB}(p_i)$.

    Before iterating over the centroid path, we do some setup for $p_1$ in Steps~\ref{step:p1cover} and~\ref{step:p1incompatible}, where the set of incompatible nodes is simply empty and the minimum cover is just the leaf in $\TB$ labelled by $p_1$. Then for-loop in Step~\ref{step:outerfor} iterates over the remaining nodes in the centroid path from bottom to top. The sets $minCover^{\TB}(\leafset^{\TA}(p_{i-1}))$ and $incompatible^{\TB}(\leafset^{\TA}(p_{i-1}))$ are obtained from the previous iteration of the loop. Then for each node $c \in sideCover^{\TB}(p_i)$, we find its coverer and then iteratively add it to the minimum cover of $p_{i-1}$ in Steps~\ref{step:findcoverer} and~\ref{step:addtocover}. Then we update the set of incompatible nodes in Step~\ref{step:updateincompatible}.

    The maximum weight of any node in the incompatible set is obtained and compared against the weight of $p_i$ in Step~\ref{step:maximumweight}. If $p_i$ is not to be deleted, then we attach the trees in the forest obtained from the previous iteration and the trees in the forests obtained from the side trees to $p_i$. Otherwise, we simply union these forests to create a larger one.

    A key optimisation here is ensuring that when we call the function recursively in Step~\ref{step:recursivecall} on a side tree $\tau \in \sigma(\TA)$, the $\TB$ is restricted such that its leafset is the same as the leafset of $\tau$. Let $\tau$ be some side tree in $\sigma(\TA)$. We reuse the technique described in \citep{jansson2018algorithms} to construct a tree $\TB||\leafset^{\tau}$ such that $\leafset^{\TB||\leafset^{\tau}} = \leafset^{\tau}$, but at the same time does not lose important weight information. To do so, we first construct $\TB|\leafset^{\tau}$ and let the weight of each node in this tree be the same as its weight in $\TB$. For each non-root node $u \in V(\TB|\leafset^{\tau})$, let $v = parent^{\TB|\leafset^{\tau}}(u)$, then if $path^{\TB}(u, v) \neq \emptyset$, we create a new node $z$ in $\TB|\leafset^{\tau}$, set $parent(u) = z$ and $parent(z) = v$. Also, we set $\weight(z) = max_{w \in path^{\TB}(u, v)} \weight(w)$. Note that this value can be obtained with an $rmqTree^{\TB}[parent(u), cFD(v)]$ query, which we denote by $rmqTree^{\TB}(u, v)$. This gives us the tree $\TB||\leafset^{\tau}$.

    \begin{figure}[ht]
        \includegraphics[scale=0.5]{specialnodes}
        \centering
        \caption[Constructing the tree $T||C$]{Part (a) shows a tree $T$ where all internal nodes are labelled with their weights. Part (b) shows the tree $T||C$, where $C = \{a, c, j\}$. Again, internal nodes are labelled with their weights. Nodes represented by diamonds in this figure are special nodes. Observe that the special node that is a parent of the leaf $j$ has weight $7$ since the nodes on the path from $j$ to the root of $T$ had weights $4, 7$ and $2$.}
        \label{fig:specialnodes}
    \end{figure}

    We build the RMQ and cFD data structure for $\TB$ to allow $rmqTree$ queries before \texttt{Filter\_Clusters} is called. Also note that since we have modified the signature of the algorithm, we need to do some processing after it finishes. In particular, we make a copy of $\TA$ and traverse this in a top-down manner, performing the $delete$ operation on the relevant nodes. Observe that this process takes $O(n)$ time. The resulting procedure \texttt{Filter\_Clusters} is presented below.
    ...
    \begin{algorithm}[!ht]
        ...
        \begin{algorithmic}[1]
            ...
            \Output The forest $\mathcal{F}_{\alpha}$ such that $\bigcup_{T' \in \mathcal{F}} \mathcal{C}(T') = \{C : C \in \mathcal{C}(\TA) \text{ and } \weight(C) > max(\{\weight(C_1) : C_1 \in \mathcal{C}(\TB) \text{ and } C_1 \not\compatible C\})\}$ and the set of nodes $minCover^{\TB}(\leafset^{\TA})$.
            ...
            \ForAll{$\tau \in \sigma(\TA)$}
                ...
                \State $sideCover^{\TB}(p_i) := sideCover^{\TB}(p_i) \cup sideCover'$ and $\mathcal{F}_{p_i} := \mathcal{F}_{p_i} \cup \mathcal{F}'$, where $\tau$ is associated with $p_i \in \pi(\TA)$
            ...
            \For{$i := 2$ \textbf{to} $\gamma$}
                ...
                \ForAll{$u \in sideCover^{\TB}(p_i)$}
                    ...
                    \State $cover :=$ \texttt{Add\_To\_Cover}$(cover, u)$
    ...
    We now go ahead to formally prove the above claim. To do so, we simply show that a node that is deleted at some point of time in the algorithm will never be added to $incompatible$ again.
    \newline

    \begin{numremovednodes}
        \label{lem:numremovednodes}

        Given trees $\TA$ and $\TB$, where $\leafset^{\TA} = \leafset^{\TB} = L$, if a node $u \in V(\TB)$ is removed from $incompatible$ in some recursive call to \texttt{Filter\_Clusters}, it will never be added to any other $incompatible$ set.

        \begin{proof}
            Let $\tau$ be some subtree of $\TA$ such that $u$ is removed from $incompatible$ when \texttt{Filter\_Clusters} is called on $\tau$. Then, by Lemma~\ref{lem:incompatibilityrecursive}, there must be some ancestor of $u$ which is in $minCover^{\TB}(\leafset^{\tau})$. Let $\tau'$ be another subtree of $\TA$ such that $u$ is added to $incompatible$ during the call to \texttt{Filter\_Clusters} on $\tau$ and this call is made after the previous one. Then for some nodes $p_i \in \pi(\tau')$, $c \in sideCover^{\TB}(p_{i+1})$,  $\leafset^{\TB}(u)$ is incompatible with $\leafset^{\TA}(p_i) \cup \leafset^{\TB}(c)$ since nodes are added to $incompatible$ as we iterate over the side cover. But then $\leafset^{\tau} \cap (\leafset^{\TA}(p_i) \cup \leafset^{\TB}(c)) \neq \emptyset$. Given the way \texttt{Filter\_Clusters} is structured, $\tau$ must be either a subtree of $\TA[p_i]$ or of one of the side trees of $p_{i+1}$. If the former, then $\leafset^{\TB}(u) \subseteq \leafset^{\TA}(p_i)$, so it cannot be incompatible. If the latter, then since some ancestor of $u$ is in the minimum cover of $\leafset^{\tau}$, $\leafset^{\TB}(u) \subseteq \leafset^{\TB}(c)$. Here also it cannot be incompatible, hence we have a contradiction.
        \end{proof}
    \end{numremovednodes}
    ...
    \begin{numaddednodes}
        ...
        Given trees $\TA$ and $\TB$, where $\leafset^{\TA} = \leafset^{\TB} = L$, the number of additions to $incompatible$ in the call to \texttt{Filter\_Clusters}$(\TA, \TB)$ is bounded by $2 \times |V(\TB)|$.

        \begin{proof}
            By Lemma~\ref{lem:numremovednodes}, no node is deleted from $incompatible$ twice. Thus, analysing very loosely, during this call to \texttt{Filter\_Clusters}, a node could at best be added, removed and added again to $incompatible$. Thus total number of additions $\leq 2 \times |V(\TB)|$.
        \end{proof}
    \end{numaddednodes}
    ...
    \subsection{The $findCoverer$ and $addToCover$ operations}
    ...
    Note that we have to make a query of the form $\leafset^{T}(v) \subseteq Cu$. However, recall that we construct trees of the form $\TB||\leafset^{\tau}$, which removes some leaves. Hence, we need to keep track of all nodes $u$ in any $\TB||\leafset^{\tau}$ where $\leafset^{\TB}(u) \neq \leafset^{\TB||\leafset^{\tau}}(u)$. This is easy to do while we're forming the tree $\TB|\leafset^{\tau}$; all such nodes are marked $spoiled$. Note that any special node is certainly $spoiled$. For example, in Figure~\ref{fig:specialnodes}(b), all internal nodes are spoilt. Thus, our check is updated to checking if $\leafset^{T}(v) \subseteq Cu$ and $v$ is not $spoiled$.

    The \texttt{Add\_To\_Cover} procedure is also easy to define using Lemma~\ref{lem:mincoverrecursive}, as done in Algorithm~\ref{alg:addtocover}.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            \Input For some tree $T$ leaf-labelled by $L$ and cluster $C \subseteq L$, takes in the set $M = minCover^{T}(C)$, some node $u \in V(T)$ and the node $v = coverer^{T, Cu}(u)$, where $\leafset^{T}(u) \cap C = \emptyset$ and $Cu = C \cup \leafset^{T}(u)$.

            \Output $minCover^{T}(Cu)$.

            \State \Return $M \cup \{v\} - \bigcup_{w \in path^{T}(u, v]} children(w)$
        \end{algorithmic}
    \end{algorithm}

    We now show that the two algorithms \texttt{Find\_Coverer} and \texttt{Add\_To\_Cover} run in amortized $O(1)$ time, proving Lemma~\ref{lem:mincoverruntime}.
    ...
    \begin{proof}[Proof of Lemma~\ref{lem:mincoverruntime}]
        ...
        When \texttt{Find\_Coverer} is called, we assign the node $u$ three units of credit. If the base case is hit, then this credit is stored in $u$. If the recursive case is hit, then the three units are assigned to the recursive call. But we still need to pay for making the counter check. To do so, we use a credit from one of the siblings of $u$. Note that this sibling must already belong to $M$, otherwise we would not hit the recursive case. Since it already belongs to $M$, it would contain some credit that we can use.

        When \texttt{Add\_To\_Cover} is called, we use the credit stored in the nodes. Since for each $w \in path^{T}(u, v]$, all nodes in $children(w)$ except for one must have already been in $M$, we use 2 of the credits assigned to these to delete them. 1 credit is used in deleting the node itself and 1 to delete the one sibling that was not in $M$. This uses up all the three credits assigned to the node during \texttt{Find\_Coverer}.

        Thus, it clear that two operations together cost amortized $O(1)$ time.
    \end{proof}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            ...
            \If{$\leafset^{T}(l_C) = C$ or $l_C = l_{Cu}$}
                \State \Return $I$
            \Else
    ...
    Note that we again need to take $spoiled$ nodes into account when making the check for $\leafset^{T}(l_C) = C$.
    ...
    \begin{proof}[Proof of Lemma~\ref{lem:incompatibilityruntime}]
        ...
        The incompatible nodes are stored in the Brodal queue of \citep{brodal1995fast}, described in the proof of Lemma~\ref{lem:filterclustersruntime}.
    ...
    We generated input trees according to two different scenarios, exactly as in \citep{jansson2018algorithms}. The first scenario, ``Scenario 1'', is the situation where trees share a large percentage of clusters, which is largely realistic. On the other hand, the second scenario, ``Scenario 2'', generates completely independent trees, which represents the other end of the spectrum. In particular, the process for generating $k$ trees with leaf labels $\{1, 2, \dots, n\}$ under the two scenarios is:
    ...
    \begin{table}[!ht]
        \captionsetup{font=scriptsize,justification=justified,margin=0cm}
        % \scriptsize
        \fontsize{9}{11}\selectfont
        \begin{minipage}{0.48\textwidth}
            \centering
            \caption{Weighting time, $n = 1000$, varying $k$}
            \label{tab:weightk1}
        ...
        \begin{minipage}{0.48\textwidth}
            \centering
            \caption{\texttt{Filter\_Clusters} time, $n = 1000$, varying $k$}
            \label{tab:filterk1}
            \begin{tabular}{c||ccccc}
                $k$ & $n\,log\,n$ & $n\,log^2n$\\
                \hline\hline
        ...
        \begin{minipage}{0.48\textwidth}
            \centering
            \caption{Weighting time, $k = 100$, varying $n$}
            \label{tab:weightn1}
            \begin{tabular}{c||ccccc}
        ...
        \begin{minipage}{0.48\textwidth}
            \centering
            \caption{\texttt{Filter\_Clusters} time, $k = 100$, varying $n$}
            \label{tab:filtern1}
            \begin{tabular}{c||ccccc}
                $n$ & $n\,log\,n$ & $n\,log^2n$\\
                \hline\hline
