...
\begin{document}
    ...
    Now, we observe some properties of $T||C$. First, we extend the definition $pure$, introduced in Subsection~\ref{subsec:mincover}, to trees with dummy nodes. Let $T||C$ be some tree containing dummy nodes and take any cluster $C' \subseteq C$. Any node $u \in V(T||C)$ belongs to $pure^{T||C}(C')$ iff $\leafset^{T||C}(u) \subseteq C'$ and the subtree rooted at $u$ does not contain any dummy nodes. Intuitively, the presence of dummy nodes indicates that some leaves were removed when creating $T||C$ from $T$. Thus, our definition of $pure^{T||C}(C')$ guarantees that the pure nodes have retained all information from $T$. Observe that $pure^{T||C}(C') = pure^{T}(C')$, since the pure nodes belong to both $T$ and $T||C$.

    As part of our algorithm we further restrict already restricted trees. We now show that the $(T||C)||C' = T||C'$.
    \newline

    \begin{furtherrestriction}
        ...
        \end{proof}
    \end{furtherrestriction}

    Observe that for a tree $T$ and nodes $u, v \in T$, where $v$ is an ancestor of $u$, $rmqTree^{T}(u, v) = rmqTree^{T}[parent(u), cFD(v)]$. Thus we can answer such queries using RMQ and cFD structures. Additionally, as shown in the proof for Lemma~\ref{lem:furtherrestriction}, when constructing $(T||C)||C'$, we can actually make the $rmqTree$ queries against $T$ instead $T||C$. Thus we can construct the RMQ and cFD structures only once on the original $T$ and use these for all subsequent restrictions.

    \subsection{The \texttt{Filter\_Clusters} algorithm}
    \label{subsec:filterclusters}

    We redefine \texttt{Filter\_Clusters} to take trees $\TA$ and $\TB$ where $\leafset^{\TA} \subseteq \leafset^{\TB} = L$ and return a representation of the cluster collection $\{C : C \in \mathcal{C}(\TA) \text{ and } \weight(C) > max(\{\weight(C_1) : C_1 \in \mathcal{C}(\TB) \text{ and } C_1 \not\compatible C\})\}$. This obeys the original definition when $\leafset^{\TA} = \leafset^{\TB}$, since the root of $\TA$ will be compatible with $\TB$ and so we can represent the above cluster collection as a tree $T$. However, if $\leafset^{\TA} \subset \leafset^{\TB}$, then the cluster associated with the root of $\TA$ might not be in the final cluster collection. Thus, in this case, we represent the output with a forest $\mathcal{F}$, such that the overall cluster collection of the trees in the forest, i.e. $\bigcup_{T \in \mathcal{F}} \mathcal{C}(T)$, gives us the set we need.

    We extend the definition of \texttt{Filter\_Clusters} to weighted restricted trees. Given trees $\TA$ and $\TB$ where $\leafset^{\TA} \subseteq \leafset^{\TB}$ and $\TB$ is not a weighted restricted tree, we define \texttt{Filter\_Clusters}$(\TA, \TB||\leafset^{\TA}) =$ \texttt{Filter\_Clusters}$(\TA, \TB)$.

    Finally, for any node $u \in V(\TA)$, define $incompatible^{\TB}(u)$ to be $incompatible^{\TB}(\leafset^{\TA}(u))$. As discussed earlier, the idea is to find the maximum weight of any node in $incompatible^{\TB}(u)$ for each node $u \in V(\TA)$.

    Our version of \texttt{Filter\_Clusters} also returns $minCover^{\TB}(\leafset^{\TA})$ for ease of processing.
    ...
    More specifically, Step~\ref{step:recursivecall} first recursively calls, for each $\tau \in \sigma(\TA)$, \texttt{Filter\_Clusters}$(\tau, \TB||\leafset^{\tau})$. From these calls, we can obtain the set $\bigcup_{\tau \in \sigma(p_i)} minCover^{\TB}(\leafset^{\tau})$ for each $p_i \in \pi(\TA)$, used in Step~\ref{step:innerfor}. This set is denoted by $sideCover^{\TB}(p_i)$. The calls also enable us to obtain, for each $\tau \in \sigma(\TA)$, the forest returned by the recursive call, denoted by $\mathcal{F}_{\tau}$. This is used in Step~\ref{step:unionforests}. Note that in order to build the trees $\TB||\leafset^{\tau}$, we construct RMQ and cFD data structures on $\TB$ before the outermost call to \texttt{Filter\_Clusters}.
    ...
    \begin{algorithm}[!ht]
        ...
        \begin{algorithmic}[1]
            ...
            \State Construct $\TB||\tau$ for each $\tau \in \sigma(\TA)$.
            \label{step:constructrestricted}

            \State Run \texttt{Filter\_Clusters}$(\tau, \TB||\tau)$ for each $\tau \in \sigma(\TA)$.
            ...
            \State $\mathcal{F} := \{\text{tree consisting of only the leaf } p_1\}$
    ...
    Before analysing \texttt{Filter\_Clusters}, we show that it is possible to build all the weighted restricted trees in $O(n)$ time.
    \newline

    \begin{restrictedweightedruntime}
        \label{lem:restrictedweightedruntime}

        For some trees $\TA$ and $\TB$ where $\leafset^{\TA} = \leafset^{\TB} = L$ and $n = |L|$, the trees $\TB||\leafset^{\tau}$ for each $\tau \in \sigma(\TA)$ can be constructed in $O(n)$ time, if RMQ and cFD structures on $\TB$ are given.

        \begin{proof}
            Since the side trees $\sigma(\TA)$ give a partition of $L$, the trees $\TB|\leafset^{\tau}$ for each $\tau \in \sigma(\TA)$ can be constructed in $O(n)$ time total using Lemma 5.2 of \cite{farach1995fast}. From these, $\TB||\leafset^{\tau}$ can be constructed in further $O(n)$ time since we can carry out $rmqTree$ queries in constant time, as we have RMQ and cFD structures on the original $\TB$ and by Lemma~\ref{lem:rmqquery} and the explanation given in Subsection~\ref{subsec:restrictedweighted}.
        \end{proof}
    \end{restrictedweightedruntime}
    ...
    \begin{filterclustersruntime}
        ...
        \begin{proof}
            The centroid path can be computed in Step~\ref{step:centroidpath} in $O(n)$ time.  The $lca$ data structure in Step~\ref{step:preprocesslca} can also be constructed in $O(n)$ time, by Lemma~\ref{lem:lca}. Step~\ref{step:recursivecall} takes $O(n)$ time, by Lemma~\ref{lem:restrictedweightedruntime}.
    ...
    \begin{impurenodesincompatiblenondummy}
        ...
        \end{proof}
    \end{impurenodesincompatiblenondummy}

    \medskip
    \begin{impurenodesincompatibledummy}
        ...
        \end{proof}
    \end{impurenodesincompatibledummy}
